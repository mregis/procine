...
class RememberMeFilter extends sfFilter
{
  ...
  {
    $cookieName = sfConfig::get('app_remember_cookie_name', 'sfRemember');
    ...
    {
      $q = Doctrine::getTable('RememberKey')->createQuery('r')
            ->innerJoin('r.Usuario u')
            ->where('r.remember_key = ?', $cookie);

      if ($q->count())
      {
        $this->context->getUser()->signIn($q->fetchOne()->Usuario);
      }
    }
